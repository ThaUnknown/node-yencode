...
export const decode = y.decode ?? function (data: Buffer, stripDots: boolean) {
...
export const minSize = function (length: number, lineSize: number) {
  if (!length) return 0
  return length + // no characters escaped
      2 * Math.floor(length / (lineSize || 128)) // newlines
}
export const maxSize = function (length: number, lineSize: number, escRatio = 2) {
  if (!length) return 0
  if (escRatio !== 2) escRatio++

  if (escRatio < 1 || escRatio > 2) { throw new Error('yEnc escape ratio must be between 0 and 1') }
...
export const encode = y.encode ?? function (data: Buffer, lineSize: number, columnOffset: number) {
...
export const post = function (filename: string, data: Buffer, lineSize = 128) {
