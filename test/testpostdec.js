...
import * as y from '../index.ts'

const toBuffer = (Buffer.alloc ? Buffer.from : Buffer)

const assertObjectHas = function (obj, required) {
  for (const k in required) {
    if (required[k] === undefined || required[k] === null) { assert(obj[k] === required[k]) } else if (typeof required[k] === 'object') // recurse to sub-objects
    { assertObjectHas(obj[k], required[k]) } else { assert.equal(obj[k], required[k], k) }
  }
}
...
post = y.fromPost(toBuffer(postData))
...
// test parse errors
post = y.fromPost(toBuffer('invalid post'))
assert.equal(post.code, 'no_start_found')
post = y.fromPost(toBuffer('=ybegin abc=def'))
