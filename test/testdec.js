...
const ord = function (c) {
  return c.charCodeAt(0)
}; const chr = String.fromCharCode
// slow reference yEnc implementation
const refYDec = function (src, findEnd) {
  const ret = []
  if (findEnd && chr(src[0]) == '=' && chr(src[1]) == 'y') return allocBuffer(0)
  for (let i = 0; i < src.length; i++) {
    switch (chr(src[i])) {
      case '\r':
        if (findEnd && chr(src[i + 1]) == '\n' && chr(src[i + 2]) == '=' && chr(src[i + 3]) == 'y') { return toBuffer(ret) }
      case '\n': continue
      case '=':
        i++
        if (i < src.length) { ret.push((src[i] - 42 - 64) & 0xFF) }
        if (chr(src[i]) == '\r') i--
        continue
    }
...
const refYDecRaw = function (src, findEnd) {
  // undo NNTP layer
  const data = []
  let i = 0
  if (src[0] == ord('.')) {
...
const doTest = function (msg, data, expected) {
  data = toBuffer(data)

  let prepad = 48; let postpad = 48
  if (data.length > 1024) {
    prepad = 1
    postpad = 1
  }
  ...
  for (var i = 0; i < prepad; i++) {
    ...
    for (var j = 0; j < postpad; j++) {
      testFuncs.forEach(function (f) {
        const post = allocBuffer(j)
        post.fill(1)

        const testData = Buffer.concat([pre, data, post])
        let x
        if (expected === undefined) x = f.r(testData).toString('hex')
        else x = toBuffer(expected).toString('hex').replace(/ /g, '')
        let actual = f.a(testData).toString('hex')
        if (actual != x) {
...
for (var i = 1; i < b.length; i += 2) { b[i] = 64 }
doTest('Long all escaped nulls', b)
b.fill('='.charCodeAt(0))
for (var i = 0; i < b.length; i += 2) { b[i] = 64 }
...
const randStr = function (n) {
  ...
  for (let i = 0; i < n; i++) { ret[i] = ord(charset[(Math.random() * charset.length) | 0]) }
  return ret
}
for (var i = 0; i < 128; i++) {
